...
export async function bearbeiteStammdaten(
  ...
  if (!stammdaten) return { ok: false, fehler: 'Noch keine Stammdaten vorhanden – der Kunde hat die Journey nicht abgeschlossen.' }
  ...
  if (eingabe.steuer_id !== undefined && eingabe.steuer_id.trim() !== '' && !/^\d{11}$/.test(eingabe.steuer_id.trim())) {
    return { ok: false, fehler: 'Steuer-ID muss 11-stellig sein.' }
  }
  ...
  const patch: Record<string, unknown> = {
    ...
    steuer_id: text(eingabe.steuer_id),
  ...
  const diff = bildeDiff(stammdaten as unknown as Record<string, unknown>, patch, STAMMDATEN_FELDER)
  if (!hatAenderungen(diff)) return { ok: true, hinweis: 'Keine Änderungen – alles bereits aktuell.' }
  ...
  try {
    ...
    await audit(angebotId, `admin:${session.user.id}`, 'stammdaten_bearbeitet', {
      felder: Object.keys(diff),
    })
    revalidatePath('/admin/kunden')
    revalidatePath('/admin')
